...
if (isset($_POST['simpan'])) {
    ...
    $er_up = $_FILES['foto_profil']['error'];

    if (empty($foto_profil)) {
        $foto_profil = "default.png";
    } else {
        $ext = pathinfo($foto_profil, PATHINFO_EXTENSION);
        $foto_profil = time() . "_" . uniqid() . "." . $ext;
        ...
        move_uploaded_file($tmp, $target_dir . $foto_profil);
    }
    ...
    if (empty($nis) || empty($nama) || empty($kelas) || empty($tajaran) || empty($kd_prodi) || empty($jk)) {
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO siswa (nis, nama, kelas, tahun_ajaran, kd_prodi, jenis_kelamin, foto_profil) VALUES (?, ?, ?, ?, ?, ?, ?)");
